  ...
  switch (day3) {
    ...
    function showWeather(position) {
      ...
      $.ajax({
        ...
        type: 'POST',
        error: function (xhr) {
          $("body").html("Error" + xhr.status).css({"color": "red", "font-size": "25px"});
        },
        success: function(json) {
          ...
          var windSpeed = json.currently.windSpeed + " mph";
          ...
          $("body").css({"background":"url('"+picToDisplay+"') no-repeat", //"center center fixed",
      ...
      function showLocation() {
        ...
        if (input != "") {
          ...
          var google = cors+'https://maps.googleapis.com/maps/api/place/autocomplete/json?input='
          +input+'&types=(cities)&language=en&key=AIzaSyAkvD4ks3WBs5U26lidsssihUZwpQk9sE4';
          $.ajax({
            type: 'GET',
            url: google,
            success: function(results) {
              city = results.predictions[0].structured_formatting.main_text;
              country = results.predictions[0].terms[1].value;
              $("#city_info").html(city+", "+country);
              if (input.includes(" ") == true) {
                input.replace(/\s/g, "+");
              }
              var json_url = "https://api.worldweatheronline.com/premium/v1/weather.ashx?key=903f599c2af043f48da01139170806&q="+input+"&num_of_days=2&tp=3&format=json";
              
              $.getJSON(json_url, function(json_data) {
          //       var data_calculation = new Date(json_data.currently.time * 1000);
          // var update_string = "";
          // var update_string2 = update_string.concat(data_calculation);
          // var update_hour = update_string2.slice(16, 21);
          // var time_zone = update_string2.slice(25, 31);
          var last_update = "Last update at "+json_data.data.current_condition[0].observation_time;
          tempFahrenheit = json_data.data.current_condition[0].temp_F;
          tempCelsius = json_data.data.current_condition[0].temp_C;
          var humidity = json_data.data.current_condition[0].humidity + "%";
          var windSpeed = json_data.data.current_condition[0].windspeedMiles + " mph";
          var precipitation = (json_data.data.current_condition[0].precipMM * 100).toFixed(0) + "%";
          var weather_description = json_data.data.current_condition[0].weatherDesc[0].value;
          var icon = json_data.data.current_condition[0].weatherIconUrl[0].value;
          var hour = d.getHours();
          ...
          $("#precip").html(precipitation);
            ...
            error: function(xhr) {
              alert(xhr.statusText.replace(xhr.statusText[0], xhr.statusText[0].toUpperCase())+ "!\nCORS has failed!");
            }
          });
        } else {
